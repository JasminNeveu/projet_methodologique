...
Pour écrire les définitions/propositions:

\begin{definition}\label{def_pvalue}
My definition
\end{definition}

Avec une citation:

\begin{prop}[Proposition 1.1 in~\cite{Gao}]\label{prop_pvalue}

\end{prop}

Labelling equations allows to cite them in the text: As we showed in Proposition~\ref{prop_pvalue}...
...
In that case we cite them using~\eqref{eq1}.

\section{Compréhension du problème}
Les méthodes de clustering ne disposent pas, en général, de garanties théoriques \deleted{fortes} sur les partitions qu'elles génèrent dans les situations pratiques usuelles.
Pour s'assurer que les résultats sont cohérents avec les attentes \Javier{We don't usually have `attentes' in practice... Plutôt on cherche à avoir des garanties sur les différences réelles entre les groupes.}, \replaced{une possibilité serait de}{il est très fréquent de} réaliser des tests d'égalité des moyennes parmi les différents clusters.
Cependant, un simple z-test ou test de Wald mène à une inflation de l'erreur de type I~\cite{Gao}. En effet, le choix \replaced{de l'hypothèse nulle}{des tests et des comparaisons} se fait après avoir observé les données (après le passage de l'algorithme de clustering), donc de manière adaptative.
...
\begin{table}[h]
  ...
  \begin{tabular}{|l|l|}
    ...
    \hline
    1. \replaced{Modèle}{Hypothèse} sur la loi \added{$\mathcal{P}$} de $X$\added{: $\mathcal{P}\in\mathcal{M}$.}        & 1. \replaced{Modèle}{Hypothèse} sur la loi \added{$\mathcal{P}$} de $X$\added{: $\mathcal{P}\in\mathcal{M}$.}         \\
    2. Choix de la question $Q$\added{: $\mathcal{P}\in\mathcal{M}_0\subset\mathcal{M}$?}           & 2. Observation des données             \\
    3. \replaced{Answer $Q$}{Tests} en utilisant \replaced{$X$}{$X \rightarrow H_0$}
                                         & 3. Choix de la question $Q(X)$\added{: $\mathcal{P}\in\mathcal{M}_0(X)\subset\mathcal{M}$?}          \\
                                          & 4. \replaced{Answer $Q(X)$}{Tests} en utilisant \replaced{$X$}{$X \rightarrow H_0$} \\
    \hline
  \end{tabular}
...
Pour savoir si une variable explicative permet d'expliquer une varibale cible au travers d'un modèle de regression linéaire, on effectue un test de significativité du coefficient à associé à cette variable.
On test s'il est significativement différent de 0.
Dans le cadre d'une regression linéaire, on se situe bien dans le cadre de l'inférence classique.
En revanche, la régression Lasso, la pénalisation $L_1$ permet de selectionner des variables.
Ainsi tout les coefficients ne sont pas à tester, seuls ceux qui n'ont pas été selectionner le sont.
On ne peut donc pas définir par avance quels coefficients tester car il dépendent de cette regression Lasso
On se retrouve donc bien dans une situation d'inférence sélective.
Lors du cours de \textit{Programmation Algotithmique} encadré par Brian Staber, nous avons pu coder un algorithme de descente proximale.
On a simulé un jeu de donnée de la sorte:

Soient $x_1, \dots, x_8$ des variables aléatoires gaussiennes centrées, de matrice de corrélation $(R_{ij})_{1 \le i,j \le 8}$ définie par
$R_{ij} = 0.5^{\lvert i-j\rvert}, $ pour $ 1 \le i,j \le 8.$
La réponse est modélisée par $y = \beta^\top x + 3\varepsilon, $
où $\varepsilon \sim \mathcal{N}(0,1)$ et
$\beta = (3,\; 1.5,\; 0,\; 0,\; 2,\; 0,\; 0,\; 0)^\top.$
On génère un échantillon de taille $n = 100$.
On obtient le chemin de régularisation suivant

\begin{figure}[h]
  \centering
  \includegraphics[width = 0.7\textwidth]{images/lasso_path.png}
  \caption{Chemin de régularisation des coefficients obtenus par algorithme Lasso. On remarque que les certains coefficients sont nuls et que 3 autres s'approchent des valeurs théoriques (3,1.5,2).}
  \label{fig:lasso_path}
\end{figure}

J'ai chosi de tester la significativité du 3eme coefficient, qui en théorie est nul (j'aurais très bien pu choisir le 4, 6,7 ou 8eme).
Si $\lvert \hat{b_3} \rvert > 1e^{-6}$ alors j'estime que la 3eme variable est selectionnée, dans ce cas je calcul la \textit{p-valeur} associée au test de significativité. 
On obtient la fonction de répartition suivante de la \textit{p-valeur}.
\clearpage
\begin{figure}[h]
  \centering
  \includegraphics[width = 0.7\textwidth]{images/pvalues_lasso.png}
  \caption{Fonction de répartition des \textit{p-valeur} obenutes par tests de significativité du 3eme coefficient obtenu par une regression Lasso. La fonction de répartition à été calculées à partir de $M = 2000$ simulations.}
